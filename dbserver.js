...
app.post('/change.html/getUsercorpus',urlencodedParser,function(req,res){
  var uuid;
  //根据userid
  var sql="SELECT * FROM coupletchatbot_corpus WHERE userid='"+req.body.user_id+"'";
...
app.post('/change.html/saveUsercorpus',urlencodedParser,function(req,res){
  var saveSql="UPDATE coupletchatbot_corpus SET first_couplet = ?,second_couplet = ?,timestamp = ?,status = ? WHERE id=? AND userid=?";
  var SqlParams = [req.body.first_couplet,req.body.second_couplet,req.body.timestamp,1,req.body.id,req.body.user_id];
  console.log(saveSql)
